...
namespace Lab3
{
    ...
    {        
        ...
        public void InsertAt(int j, ResearchTeam researchTeam)
        {
            ...
            if (_researchTeamList.Count < j)
            {
                _researchTeamList.Add(researchTeam);
                ResearchTeamAdded?.Invoke(_researchTeamList, new TeamListHandlerEventArgs(NameOfCollection, "Added", _researchTeamList.Count - 1));
            }
            else
            {
                _researchTeamList.Insert(j, researchTeam);
                ResearchTeamInserted?.Invoke(_researchTeamList, new TeamListHandlerEventArgs(NameOfCollection, "Inserted", _researchTeamList.IndexOf(researchTeam,j)));
            }
        }
